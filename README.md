...
> 桌面版发布见 https://github.com/Vvv1940905115/mihomo-desktop/releases/tag/v2.1.0 
...
| **订阅**           | 新增 / 改名改址 / 删除 / 单个更新 / 一键更新全部，显示节点数与更新时间，节点自动合并进内核配置（PROXY 组）                 |
| **连接**           | 实时连接列表、断开指定连接                                                               |
| **日志**           | SSE 实时流、按等级过滤、搜索                                                            |
...
| **代理** | 按代理组展示节点，点击切换；延迟测速分三色：<800ms 绿、≥800ms 黄（可用）、失败红（禁用）                    |
